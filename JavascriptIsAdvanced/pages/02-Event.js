...
document.querySelector('#big').addEventListener('click', function(event){
    console.log('Big click', event.target.style.background);

    return  event.target.style.background = event.target.style.background != 'red' ? 'red' : '#f0f';
}, true);
...
document.querySelector('#small-2').addEventListener('click', function(event){
    ...
    backg = this.style.background != 'green' ? 'green' : '#0f0'
    console.log('Small click', event);
    //
